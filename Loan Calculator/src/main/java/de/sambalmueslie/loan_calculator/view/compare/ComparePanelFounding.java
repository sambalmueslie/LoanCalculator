...
class ComparePanelFounding extends BaseComparePanel<Founding> {
	...
	protected void setup(final Set<Founding> elements) {
		...
		comparePane.getChildren().add(
				addCompareFunction(I18n.get(I18n.TEXT_TOTAL_INTEREST), Founding::getTotalInterest));
		comparePane.getChildren().add(addCompareFunction(I18n.get(I18n.TEXT_TOTAL_AMOUNT), Founding::getAmount));
		comparePane.getChildren().add(addCompareFunction(I18n.get(I18n.TEXT_TERM), Founding::getTerm));
		comparePane.getChildren().add(addCompareFunction(I18n.get(I18n.TEXT_RISK_CAPITAL), Founding::getRiskCapital));

		final Function<Founding, RedemptionPlanEntry> dataGetterFunction = (f -> f.getRedemptionPlan().get(1));
		final SeriesDefinition<RedemptionPlanEntry, Double> sd1 = new SeriesDefinition<>(I18n.get(I18n.TEXT_INTEREST),
				RedemptionPlanEntry::getInterest);
		final SeriesDefinition<RedemptionPlanEntry, Double> sd2 = new SeriesDefinition<>(
				I18n.get(I18n.TEXT_REDEMPTION), RedemptionPlanEntry::getRedemption);
	...
	private Node createDetailsPanel(final Founding founding) {
		...
		final GenericPieChart<Founding, Loan> totalAmountChart = new GenericPieChart<>(Founding::getLoans, Loan::getAmount,
				I18n.get(I18n.TEXT_TOTAL_AMOUNT), founding);
